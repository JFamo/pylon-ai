...
class Chevron:
	...
	def __init__(self):
		...
		self.hr_buildPriorities = {PROBE: 1, NEXUS: 10, PYLON: 4, GATEWAY: 3, STARGATE: 3, ZEALOT: 1, SENTRY: 1, STALKER: 1, ASSIMILATOR: 3, CYBERNETICSCORE: 5, FORGE: 5, VOIDRAY: 2, COLOSSUS: 2, FLEETBEACON: 4, TWILIGHTCOUNCIL: 5, PHOTONCANNON: 2, TEMPLARARCHIVE: 4, DARKSHRINE: 4, ROBOTICSBAY: 4, ROBOTICSFACILITY: 3, HIGHTEMPLAR: 2, DARKTEMPLAR: 2, PHOENIX: 2.5, CARRIER: 3, WARPPRISM: 2, OBSERVER: 4, IMMORTAL: 2, ADEPT: 1, ORACLE: 1, TEMPEST: 2, DISRUPTOR: 1} # This should be situational, generalize for now
		# Priority values for all upgrades
		self.hr_upgradePriorities = {RESEARCH_PSISTORM:5, RESEARCH_WARPGATE: 5, RESEARCH_BLINK: 5, "DEFAULT": 5}

		# Supply ratio of units for build
		self.hr_unitRatio = {}
		self.hr_unitRatio[ZEALOT] = 0.2
		self.hr_unitRatio[STALKER] = 0.3
		self.hr_unitRatio[SENTRY] = 0.05
		self.hr_unitRatio[VOIDRAY] = 0.2
		self.hr_unitRatio[COLOSSUS] = 0.05
		self.hr_unitRatio[HIGHTEMPLAR] = 0.0
		self.hr_unitRatio[DARKTEMPLAR] = 0.0
		self.hr_unitRatio[PHOENIX] = 0.0
		self.hr_unitRatio[CARRIER] = 0.0
		self.hr_unitRatio[WARPPRISM] = 0.0
		self.hr_unitRatio[OBSERVER] = 0.01
		self.hr_unitRatio[IMMORTAL] = 0.05
		self.hr_unitRatio[ADEPT] = 0.0
		self.hr_unitRatio[ORACLE] = 0.0
		self.hr_unitRatio[TEMPEST] = 0.0
		...
		#self.hr_upgradeTime[RESEARCH_WARPGATE] = [CYBERNETICSCORE,420]
		self.hr_upgradeTime[CYBERNETICSCORERESEARCH_PROTOSSAIRWEAPONSLEVEL1] = [CYBERNETICSCORE,600]
		...
		self.hr_upgradeTime[CYBERNETICSCORERESEARCH_PROTOSSAIRARMORLEVEL1] = [CYBERNETICSCORE,700]
		...
		self.hr_upgradeTime[RESEARCH_CHARGE] = [TWILIGHTCOUNCIL,1200]
		self.hr_upgradeTime[RESEARCH_BLINK] = [TWILIGHTCOUNCIL,800]
		self.hr_upgradeTime[RESEARCH_ADEPTRESONATINGGLAIVES] = [TWILIGHTCOUNCIL,1500]
		self.hr_upgradeTime[RESEARCH_PHOENIXANIONPULSECRYSTALS] = [FLEETBEACON,2000]
		self.hr_upgradeTime[RESEARCH_PSISTORM] = [TEMPLARARCHIVE,1600]
		self.hr_upgradeTime[RESEARCH_GRAVITICBOOSTER] = [ROBOTICSBAY,1700]
		self.hr_upgradeTime[RESEARCH_GRAVITICDRIVE] = [ROBOTICSBAY,1600]
		self.hr_upgradeTime[RESEARCH_EXTENDEDTHERMALLANCE] = [ROBOTICSBAY,1500]
	...
	def commit(self):
		...
		print("Committed " + self.name + " : " + str(self.score))